...
# Hardcoded values
DATASET_ID = "Livechat_Data"  # Replace with your dataset ID
TABLE_ID = "Livechat_Raw_Data_V2"  # Replace with your table ID
CREDENTIALS_PATH = "C:\\Users\\PM Shift\\OneDrive\\sc-v1\\youtube-cenix-1e95c7a6cd7b.json"
...
def get_agent_alias(users):
    ...
    # Map based on email and name    # Map based on email and name
...
def format_time_from_iso(iso_timestamp):
    """Helper function to convert ISO timestamp to TIME format in PH time"""
    try:
        dt = datetime.strptime(iso_timestamp, "%Y-%m-%dT%H:%M:%S.%fZ")
        ph_time = dt + timedelta(hours=8)  # Convert to PH time
        return ph_time.strftime("%H:%M:%S")  # Returns time in 24-hour format
    except (ValueError, TypeError):
        return "00:00:00"  # Default to "00:00:00" instead of "N/A"
...
def calculate_chatting_time(started_time, end_time):
    ...
    try:
        ...
        pass
    return "00:00:00"  # Default to "00:00:00" if calculation fails
...
def process_chat_to_row(chat):
    """Convert a single chat's data into a BigQuery row"""
    row = {}
    ...
    # Initialize times
    started_time = thread.get('created_at')
    end_time = thread.get('ended_at')
    
    # If no created_at, try to get from first event
    if not started_time and events:
        first_event = events[0]
        started_time = first_event.get('created_at')
    ...
    row['Chatting Time'] = calculate_chatting_time(started_time, end_time)
    ...
    if contact_date != 'N/A':
        try:
            ...
            ph_time = dt + timedelta(hours=8)  # Convert to PH time
            row['Contact Date'] = ph_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
        except ValueError:
...
def upload_to_bigquery(data):
    """Upload data to BigQuery using a service account JSON key file"""
    ...
    # Ensure TIME fields are "00:00:00" instead of NULL
    for row in data:
        for time_field in ['Started Time', 'EndTime', 'Chatting Time']:
            if not row[time_field]:  # If the field is blank or None
                row[time_field] = "00:00:00"
    
    errors = client.insert_rows(table, data)
    if errors:
        print(f"Errors occurred while inserting rows: {errors}")
    else:
        print(f"Successfully uploaded {len(data)} rows to BigQuery")

def download_chat_transcript(account_id, token, chat_id=None, custom_range=False):
    """Download and process chat transcripts directly to BigQuery"""
    ...
    # First, save all raw chat data to a temporary file
    raw_chats_file = 'raw_chats_data.json'
    all_chats = []
    page_id = None
    total_chats = 0
...
def process_raw_chats(raw_chats_file, output_file):
    ...
    if not output_file:
        ...
        output_file = f'chat_transcript_{timestamp}.csv'
...
def validate_time(time_str):
    try:
        ...
        if not (0 <= time_obj.hour < 24):
            return '00:00:00'
        return time_str
    except ValueError:
        # Return default time for invalid formats
        return '00:00:00'

def insert_data_to_bigquery(client, table_id, data):
    errors = []
    successful_inserts = 0
    
    # List of all possible time fields
    time_fields = [
        'time_column', 'Started Time', 'EndTime', 
        'Chatting Time', 'Queued Time', 'Came from'
    ]
    
    for i, row in enumerate(data):
        try:
            # Validate and clean all time fields
            for field in time_fields:
                if field in row and row[field]:
                    original_time = row[field]
                    row[field] = validate_time(row[field])
                    if row[field] != original_time:
                        print(f"Fixed invalid time in row {i}, field {field}: {original_time} -> {row[field]}")
            ...
            # Continue processing next rows even if one fails
    
    if errors:
        print(f"\nErrors occurred while inserting {len(errors)} rows")
        print(f"Successfully inserted {successful_inserts} rows")
        print(f"Error details: {errors}")
    else:
        print(f"\nSuccessfully inserted all {len(data)} rows")
